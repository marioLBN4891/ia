...
public class DatabaseHelper extends SQLiteOpenHelper
{
	...
	private void creaTabelleDatabase(SQLiteDatabase db) {
		...
		String sqlReportTable = "CREATE TABLE {0} ({1} INTEGER PRIMARY KEY, {2} TEXT NOT NULL, {3} TEXT NOT NULL, {4} TEXT NOT NULL, {5} INTEGER NOT NULL);";
		db.execSQL(MessageFormat.format(sqlReportTable, ReportTable.TABLE_NAME, BaseColumns._ID, ReportTable.AZIONE, ReportTable.PROLOG, ReportTable.ITEM, ReportTable.STATO));
	...
	private void popolaOggettiTable(SQLiteDatabase db) {
		...
		Oggetto.setConfigurazione(db, "Caffè", Oggetto.DISPENSA, 0, "caffe");
	...
	private void popolaComponentiTable(SQLiteDatabase db) {
		...
		Componente.setConfigurazione(db, "Macchina del caffè", Componente.ACCESO_SPENTO, 0, "coffee_machine_choice");
